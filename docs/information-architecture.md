...
“扩展与插件”使用三级视图：Runtime Provider、Runtime Profile、平台扩展。Provider 与平台扩展从列表进入详情，详情展示身份、隔离、契约、能力协商、探针、生命周期和日志；Profile 使用列表/编辑详情并保留高级组合配置。生命周期变更只对管理员显示并由 BFF 再次强制校验。
